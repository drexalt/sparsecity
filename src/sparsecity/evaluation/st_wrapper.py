...
class ST_SPLADEModule_addTopK(nn.Module):
    def __init__(self, splade_model, tokenizer, max_length=256, top_k=128):
        super().__init__()
        self.splade_model = splade_model
        self.tokenizer = tokenizer
        self.max_length = max_length
    ...
    def forward(self, features: dict) -> dict:
        ...
        top_values, _ = torch.topk(output, k=self.top_k, dim=-1)
    ...
    def encode(self, texts, batch_size=32, **kwargs):
        ...
        with torch.inference_mode():
            outputs = self.splade_model(**inputs)
        # Top-K masking
        top_values, _ = torch.topk(outputs, k=self.top_k, dim=-1)
        threshold = top_values[..., -1, None]
        outputs = outputs * (outputs >= threshold)
